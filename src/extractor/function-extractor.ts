  ...
  async extract(
   ...
   // logger.writeResults(matches, 'matches')
     ...
     for (const match of matches) {
      ...
      const fnId = this.generateNodeId('function', null, filePath, funcNode.startPosition.row + 1, funcNode.endPosition.row + 1, funcNode.startPosition.column + 1, funcNode.endPosition.column + 1)
       let nodeId = null
      if(nodeType !== 'unknown') {
       nodeId = this.generateNodeId(nodeType, null, filePath, scope.parentNode.startPosition.row + 1, scope.parentNode.endPosition.row + 1, scope.parentNode.startPosition.column + 1, scope.parentNode.endPosition.column + 1)
      }
