...
export class TypeScriptLSPClient {
  ...
  private async initialize(): Promise<void> {
    console.log(
      'Root uri:',
      this.workspaceRoot
        ? `file://${path.resolve(this.workspaceRoot)}`
        : 'No workspace root',
    )
  ...
  private handleResponse(data: string): void {
    ...
    while (true) {
      ...
      const messageJson = this.buffer.substring(
        headerLength,
  ...
  async findFunctions(
    filePath: string,
  ): Promise<Array<{ name: string; position: Position }>> {
    const fileUri = `file://${path.resolve(filePath)}`
    const languageId = this.getLanguageId(filePath)
  ...
  async getAllSymbols(
    filePath: string,
  ): Promise<Array<{ name: string; kind: number; position: Position }>> {
    const fileUri = `file://${path.resolve(filePath)}`
    const languageId = this.getLanguageId(filePath)
  ...
  > {
    ...
    try {
      ...
      if (debug) {
        console.log(
          `DEBUG LSP: prepareCallHierarchy for ${filePath}:${position.line}:${position.character}`,
          callHierarchyItems?.length || 0,
          'items found',
        )
      }
      ...
      for (const call of outgoingCalls || []) {
        ...
        const fromRange =
          call.fromRanges && call.fromRanges.length > 0
            ? call.fromRanges[0]
            : call.to.range
        ...
        callees.push({
          ...
          callSite: {
            line: fromRange
              ? fromRange.start.line + 1
              : call.to.range.start.line + 1,
            column: fromRange
              ? fromRange.start.character + 1
