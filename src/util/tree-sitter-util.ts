...
export class TreeSitterUtil {
  public determineNodeContext(
    ...
    if(result.context === null) {
      result.context = result.parentNode.startPosition.row == 0 && result.parentNode.startPosition.column == 0 ? "Top-Level" : "Unknown Context"
    }
...
extractArrowFunctionSignature(funcNode: Parser.SyntaxNode, nameCapture?: Parser.QueryCapture): string {
  const paramsNode = funcNode.childForFieldName('parameters') || funcNode.childForFieldName('parameter')
  const returnTypeNode = funcNode.childForFieldName('return_type')
...
extractFunctionExpressionSignature(funcNode: Parser.SyntaxNode, nameCapture?: Parser.QueryCapture): string {
  const nameNode = funcNode.childForFieldName('name')
  const paramsNode = funcNode.childForFieldName('parameters')
  const returnTypeNode = funcNode.childForFieldName('return_type')
...
extractFunctionName(funcNode: Parser.SyntaxNode, nameCapture?: Parser.QueryCapture): string {
  ...
  if (funcNode.type === 'arrow_function' || funcNode.type === 'function_expression') {
    return `anonymous_${funcNode.startPosition.row + 1}_${funcNode.startPosition.column + 1}`
  }
  
  return 'unknown'
}

 extractFunctionSignature(funcNode: Parser.SyntaxNode, nameCapture?: Parser.QueryCapture): string {
 ...
 */  
public findFunctionCalls(functionNode: Parser.SyntaxNode): Array<{  
  node: Parser.SyntaxNode;  
  functionName: string;  
  arguments: Parser.SyntaxNode[];  
  line: number;  
  column: number;  
  callType: 'function_call' | 'method_call' | 'constructor_call' | 'tagged_template' | 'chained_call' | 'dynamic_call' | 'higher_order_call' | 'optional_chaining_call';  
  chainDepth?: number;  
  isAsync?: boolean;  
}> {  
  const calls: Array<{  
 ...
 */  
private searchForCallsExtended(node: Parser.SyntaxNode, calls: Array<{  
  node: Parser.SyntaxNode;  
  functionName: string;  
  arguments: Parser.SyntaxNode[];  
  line: number;  
  ...
  if (node.type === 'call_expression') {  
    ...
    if (functionField) {  
      const callInfo = this.analyzeCallExpression(node, functionField, argumentsField);  
      if (callInfo) {  
        calls.push(callInfo);  
      }  
    }  
  ...
  // Handle new expressions (constructor calls)  
    ...
    if (constructorField) {  
      const callInfo = this.analyzeNewExpression(node, constructorField, argumentsField);  
      if (callInfo) {  
        calls.push(callInfo);  
      }  
    }  
  ...
  else if (node.type === 'template_string' && node.parent?.type === 'call_expression') {  
  ...
  // Handle await expressions  
    ...
    if (expression && expression.type === 'call_expression') {  
      ...
      if (functionField) {  
        const callInfo = this.analyzeCallExpression(expression, functionField, argumentsField);  
        if (callInfo) {  
...
private analyzeCallExpression(  
  ...
  let functionName = '';  
  let callType: 'function_call' | 'method_call' | 'constructor_call' | 'tagged_template' | 'chained_call' | 'dynamic_call' | 'higher_order_call' | 'optional_chaining_call' = 'function_call';  
  let chainDepth = 0;  
  
  // Check if this is a tagged template literal  
  if (!argumentsField && callNode.childForFieldName('arguments')?.type === 'template_string') {  
...
public traceCalleeToDefinition(  
  ...
  while (currentName && !visited.has(currentName)) {  
    ...
    if (declaration.type === 'variable') {  
      const assignedValue = this.getVariableAssignedValue(declaration.parentNode);  
  ...
  const finalDefinition = chain.length > 0 ? {  
...
private getVariableAssignedValue(variableDeclaratorNode: Parser.SyntaxNode | null): Parser.SyntaxNode | null {  
  if (!variableDeclaratorNode || variableDeclaratorNode.type !== 'variable_declarator') {  
    return null;  
  }  
...
public getAllClassMembers(classNode: Parser.SyntaxNode): Array<{  
  node: Parser.SyntaxNode;  
  name: string;  
  signature: string;  
  ...
  for (let i = 0; i < classBody.namedChildCount; i++) {  
    ...
    if (member.type === 'method_definition') {  
      ...
      members.push({  
        ...
        isPrivate: nameNode.type === 'private_property_identifier' || methodName.startsWith('#'),  
    ...
    // Handle field definitions  
      ...
      if (initializerNode &&   
          (initializerNode.type === 'arrow_function' || initializerNode.type === 'function_expression')) {  
        signature = this.extractFieldFunctionSignature(member, initializerNode);  
      ...
      members.push({  
        ...
        isPrivate: propertyNode.type === 'private_property_identifier' || fieldName.startsWith('#'),  
...
private extractFieldFunctionSignature(fieldNode: Parser.SyntaxNode, functionNode: Parser.SyntaxNode): string {  
